...
use amethyst::error::{format_err, Error};
use std::collections::HashMap;

use crate::render_mesh::ExtendedBackend;
...
#[derive(Debug)]
pub struct ExtendedRenderingBundle<B: ExtendedBackend> {
    plugins: Vec<Box<dyn RenderPlugin<B>>>,
}

impl<B: ExtendedBackend> ExtendedRenderingBundle<B> {
...
impl<'a, 'b, B: ExtendedBackend> SystemBundle<'a, 'b> for ExtendedRenderingBundle<B> {
    fn build(
        mut self,
        world: &mut World,
        builder: &mut DispatcherBuilder<'a, 'b>,
    ) -> Result<(), Error> {
        builder.add(MeshProcessorSystem::<B>::default(), "extended_mesh_processor", &[]);
        // builder.add(
        ...
        builder.add_thread_local(ExtendedRenderingSystem::<B, _>::new(self.into_graph_creator()));
        Ok(())
    }
}
