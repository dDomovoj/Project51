...
use amethyst::renderer::{
    ...
    rendy::{
        ...
        memory::Write as _,
        resource::{Buffer, BufferInfo, DescriptorSet, DescriptorSetLayout, Escape, Handle as RendyHandle},
    },
...
use crate::render_material::{ITextureSet, Material, ShaderMaterial};
...
impl<B: Backend> SlottedBuffer<B> {
    fn new(
        factory: &Factory<B>, elem_size: u64, capacity: usize, usage: hal::buffer::Usage,
        ...
        Ok(Self {
            buffer: factory.create_buffer(
                ...
                amethyst::renderer::rendy::memory::Dynamic,
            )?,
            elem_size,
        })
    }
    ...
    pub fn descriptor(&self, id: usize) -> Descriptor<'_, B> {
        ...
        Descriptor::Buffer(self.buffer.raw(), Some(offset)..Some(offset + self.elem_size))
    }
    ...
    fn write(&mut self, factory: &Factory<B>, id: usize, data: &[u8]) {
        ...
        let mut mapped = self
            ...
            .unwrap();
        unsafe {
            let mut writer = mapped.write(factory.device(), 0..data.len() as u64).unwrap();
            writer.write(data);
        }
    }
}
...
#[derive(Debug)]
pub struct MaterialSub<B: Backend, T: for<'a> ITextureSet<'a>> {
    generation: u32,
...
impl<B: Backend, T: for<'a> ITextureSet<'a>> MaterialSub<B, T> {
    ...
    fn try_insert(
        &mut self, factory: &Factory<B>, world: &World, handle: &Handle<Material>,
        ...
        let (mat_storage, tex_storage) =
            <(Read<'_, AssetStorage<Material>>, Read<'_, AssetStorage<Texture>>)>::fetch(world);

        let mat = mat_storage.get(handle).unwrap();

        let has_tex =
            T::textures(mat).any(|t| !tex_storage.get(t).map_or(false, |tex| B::unwrap_texture(tex).is_some()));
        if has_tex {
            return None;
        }

        let pod = ShaderMaterial::from_material(&mat).std140();
        ...
        unsafe {
            ...
            let tex_descs = T::textures(mat).enumerate().map(|(i, t)| {
                desc_write(
                    set,
                    (i + 1) as u32,
                    texture_desc(tex_storage.get(t).unwrap(), hal::image::Layout::ShaderReadOnlyOptimal).unwrap(),
                )
            });
    ...
    pub fn insert(
        ...
        match self.materials.get_mut(id) {
            ...
            Some(MaterialState::Unloaded { generation }) if *generation == self.generation => return None,
            _ => {}
        };
    ...
    #[inline]
    pub fn bind(
        &self, pipeline_layout: &B::PipelineLayout, set_id: u32, material_id: MaterialId,
        encoder: &mut RenderPassEncoder<'_, B>,
    ) {
        match &self.materials[material_id.0 as usize] {
            MaterialState::Loaded { set, .. } => unsafe {
                encoder.bind_graphics_descriptor_sets(pipeline_layout, set_id, Some(set.raw()), std::iter::empty());
            },
